...
package org.apache.flink.table.server.gateway;
...
public interface Executor {
	 ...
	 */
	Map<String, String> getSessionProperties() throws SqlExecutionException;

	/**
	 * Lists all registered catalogs.
	 */
	List<String> listCatalogs() throws SqlExecutionException;

	/**
	 * Lists all databases in the current catalog.
	 */
	List<String> listDatabases() throws SqlExecutionException;

	/**
	 * Lists all tables in the current database of the current catalog.
	 */
	List<String> listTables() throws SqlExecutionException;

	/**
	 * Lists all user-defined functions known to the executor.
	 */
	List<String> listUserDefinedFunctions() throws SqlExecutionException;

	/**
	 * Lists all functions known to the executor.
	 */
	List<String> listFunctions() throws SqlExecutionException;

	/**
	 * Sets a catalog with given name as the current catalog.
	 */
	void useCatalog(String catalogName) throws SqlExecutionException;

	/**
	 * Sets a database with given name as the current database of the current catalog.
	 */
	void useDatabase(String databaseName) throws SqlExecutionException;
	 ...
	 */
	TableSchema getTableSchema(String name) throws SqlExecutionException;

	/**
	 * Returns a string-based explanation about AST and execution plan of the given statement.
	 */
	String explainStatement(String statement) throws SqlExecutionException;
	 ...
	 */
	ResultDescriptor executeQuery(String query) throws SqlExecutionException;

	/**
	 * Asks for the next changelog results (non-blocking).
	 */
	TypedResult<List<Tuple2<Boolean, Row>>> retrieveResultChanges(String resultId) throws SqlExecutionException;
	 ...
	 */
	ProgramTargetDescriptor executeUpdate(String statement) throws SqlExecutionException;

	/**
	 * Validates the current session. For example, it checks whether all views are still valid.
	 */
	void validateSession() throws SqlExecutionException;
