...
package org.apache.flink.table.server.gateway.local;
...
import org.apache.flink.table.server.gateway.ProgramTargetDescriptor;
...
import org.apache.flink.table.server.gateway.local.result.BasicResult;
import org.apache.flink.table.server.gateway.local.result.ChangelogResult;
import org.apache.flink.table.server.gateway.local.result.DynamicResult;
import org.apache.flink.table.server.gateway.local.result.MaterializedResult;
...
public class ExecutorImpl implements Executor {
	 ...
	 */
	public ExecutorImpl(SessionContext sessionContext, URL defaultEnv, List<URL> jars, List<URL> libraries) {
		...
		try {
			...
			throw new SqlExecutionException("Could not load Flink configuration.", e);
		}
		...
		if (defaultEnv != null) {
			...
			try {
				...
				throw new SqlExecutionException("Could not read default environment file at: " + defaultEnv, e);
			}
		} else {
	 ...
	 */
	public ExecutorImpl(SessionContext sessionContext, Environment defaultEnvironment, List<URL> dependencies, Configuration flinkConfig, CustomCommandLine<?> commandLine) {
		this.sessionContext = sessionContext;
	...
	@Override
	public void start() {
		this.executionContext = getOrCreateExecutionContext();
		this.tableEnv = executionContext.createEnvironmentInstance().getTableEnvironment();
	}

	@Override
	public Map<String, String> getSessionProperties() throws SqlExecutionException {
		final Environment env = getOrCreateExecutionContext()
	...
	@Override
	public List<String> listCatalogs() throws SqlExecutionException {
		return executionContext.wrapClassLoader(() -> Arrays.asList(tableEnv.listCatalogs()));
	}

	@Override
	public List<String> listDatabases() throws SqlExecutionException {
		return executionContext.wrapClassLoader(() -> Arrays.asList(tableEnv.listDatabases()));
	}

	@Override
	public List<String> listTables() throws SqlExecutionException {
		return executionContext.wrapClassLoader(() -> Arrays.asList(tableEnv.listTables()));
	}

	@Override
	public List<String> listUserDefinedFunctions() throws SqlExecutionException {
		return executionContext.wrapClassLoader(() -> Arrays.asList(tableEnv.listUserDefinedFunctions()));
	}

	@Override
	public List<String> listFunctions() throws SqlExecutionException {
		return executionContext.wrapClassLoader(() -> Arrays.asList(tableEnv.listFunctions()));
	}

	@Override
	public void useCatalog(String catalogName) throws SqlExecutionException {
	...
	@Override
	public void useDatabase(String databaseName) throws SqlExecutionException {
		executionContext.wrapClassLoader(() -> {
			// Rely on TableEnvironment/CatalogManager to validate input
			try {
	...
	@Override
	public TableSchema getTableSchema(String name) throws SqlExecutionException {
		try {
			return executionContext.wrapClassLoader(() -> tableEnv.scan(name).getSchema());
		} catch (Throwable t) {
	...
	@Override
	public String explainStatement(String statement) throws SqlExecutionException {
		// translate
		try {
			final Table table = createTable(executionContext, tableEnv, statement);
			return executionContext.wrapClassLoader(() -> tableEnv.explain(table));
		} catch (Throwable t) {
			// catch everything such that the query does not crash the executor
	...
	@Override
	public ResultDescriptor executeQuery(String query) throws SqlExecutionException {
		return executeQueryInternal(executionContext, query);
	}

	@Override
	public TypedResult<List<Tuple2<Boolean, Row>>> retrieveResultChanges(String resultId) throws SqlExecutionException {
	...
	public void cancelQuery(String resultId) throws SqlExecutionException {
		cancelQueryInternal(executionContext, resultId);
	}

	@Override
	public ProgramTargetDescriptor executeUpdate(String statement) throws SqlExecutionException {
		return executeUpdateInternal(executionContext, statement);
	}

	@Override
	public void validateSession() throws SqlExecutionException {
		// throws exceptions if an environment cannot be created with the given session context
		getOrCreateExecutionContext().createEnvironmentInstance();
	}
	 ...
	 */
	private synchronized ExecutionContext<?> getOrCreateExecutionContext() throws SqlExecutionException {
		if (executionContext == null || !executionContext.getSessionContext().equals(this.sessionContext)) {
			try {
				executionContext = new ExecutionContext<>(defaultEnvironment, this.sessionContext, dependencies,
	...
	private static List<URL> discoverDependencies(List<URL> jars, List<URL> libraries) {
		...
		try {
			...
			for (URL libUrl : libraries) {
				...
				if (!dir.isDirectory()) {
					throw new SqlExecutionException("Directory expected: " + dir);
				} else if (!dir.canRead()) {
					throw new SqlExecutionException("Directory cannot be read: " + dir);
				}
			...
			throw new SqlExecutionException("Could not load all required JAR files.", e);
		}
