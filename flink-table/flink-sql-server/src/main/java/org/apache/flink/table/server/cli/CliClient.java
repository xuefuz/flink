...
package org.apache.flink.table.server.cli;

import org.apache.flink.annotation.VisibleForTesting;
import org.apache.flink.table.api.TableSchema;
import org.apache.flink.table.server.SqlClientException;
import org.apache.flink.table.server.cli.SqlCommandParser.SqlCommandCall;
import org.apache.flink.table.server.config.entries.ViewEntry;
...
public class CliClient {
	...
	private void callShowCatalogs() {
		...
		try {
			catalogs = executor.listCatalogs();
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private void callUseCatalog(SqlCommandCall cmdCall) {
		try {
			executor.useCatalog(cmdCall.operands[0]);
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private void callUseDatabase(SqlCommandCall cmdCall) {
		try {
			executor.useDatabase(cmdCall.operands[0]);
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private void callDescribe(SqlCommandCall cmdCall) {
		...
		try {
			schema = executor.getTableSchema(cmdCall.operands[0]);
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private void callExplain(SqlCommandCall cmdCall) {
		...
		try {
			explanation = executor.explainStatement(cmdCall.operands[0]);
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private void callSelect(SqlCommandCall cmdCall) {
		...
		try {
			resultDesc = executor.executeQuery(cmdCall.operands[0]);
		} catch (SqlExecutionException e) {
			printExecutionException(e);
			return;
		}
	...
	private boolean callInsertInto(SqlCommandCall cmdCall) {
		...
		try {
			final ProgramTargetDescriptor programTarget = executor.executeUpdate(cmdCall.operands[0]);
